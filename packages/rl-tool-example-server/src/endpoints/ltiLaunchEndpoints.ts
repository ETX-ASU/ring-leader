...
const plateformAccess = {
  ...
  kty: "RSA",
  redirect_uri:
    "https://ring-leader-devesh-tiwari.herokuapp.com/lti-advantage-launch",
  ...
  client_id: "97140000000000177" // we get this during plateform registration
};
...
const ltiLaunchEndpoints = (app: Express): void => {
  ...
  app.post(LTI_ADVANTAGE_LAUNCH_ROUTE, requestLogger, async (req, res) => {
    ...
    getAccessToken(plateformAccess, req.body.id_token);
    await req.session.save(() => {
      console.log("session data saved");
    });
